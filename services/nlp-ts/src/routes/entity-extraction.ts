...
export async function entityRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: EntityExtractionRequest }>, reply: FastifyReply) => {
      try {
        ...
        if (!entityExtractor || !entityExtractor.extractEntities) {
          reply.code(500);
          return {
            error: 'Service Error',
        ...
        if (options.confidence_threshold) {
          result.entities = result.entities.filter(
            (entity: MedicalEntity) => entity.confidence >= options.confidence_threshold!
          );
        }
        ...
        // Log to database
        if (databaseService && databaseService.logEntityExtraction) {
          try {
        ...
        return {
          ...
          normalized_entities:
            options.include_normalized !== false ? result.normalizedEntities : undefined,
          entity_relations:
            options.include_relations !== false ? result.entityRelations : undefined,
          metadata: result.metadata,
        };
      } catch (error: any) {
  ...
  fastify.post(
    ...
    async (
      request: FastifyRequest<{ Body: BatchEntityExtractionRequest }>,
      reply: FastifyReply
    ) => {
      try {
        ...
        if (!entityExtractor.batchExtractEntities) {
          reply.code(500);
          return { error: 'Service Error', message: 'Entity extractor not available' };
        }
  ...
  fastify.get(
    '/entity-extraction/history/:documentId',
    {
      schema: {
        tags: ['Entity Extraction'],
    ...
    async (request: FastifyRequest<{ Params: { documentId: string } }>, reply: FastifyReply) => {
      try {
        ...
        const history = entityExtractor.getExtractionHistory
          ? entityExtractor.getExtractionHistory(documentId)
          : null;
