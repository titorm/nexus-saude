...
export async function clinicalProcessingRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest<{ Body: ClinicalProcessingRequest }>, reply: FastifyReply) => {
      try {
        ...
        const clinicalNLPProcessor = fastify.clinicalNLPProcessor;
        ...
        // Ensure processor implements the method
        if (!clinicalNLPProcessor || !clinicalNLPProcessor.processDocument) {
          reply.code(500);
          return { error: 'Service Error', message: 'Clinical NLP processor not available' };
        }
        ...
        if (monitoringService) {
          monitoringService.recordRequest?.(
            'clinical_processing_comprehensive',
            processingTime,
            true
          );
          monitoringService.recordClinicalProcessing?.(
  ...
  fastify.post(
    ...
    async (
      request: FastifyRequest<{ Body: BatchClinicalProcessingRequest }>,
      reply: FastifyReply
    ) => {
      try {
        ...
        if (!clinicalNLPProcessor.batchProcessDocuments) {
          reply.code(500);
          return { error: 'Service Error', message: 'Clinical NLP processor not available' };
        }
        ...
        for (const [docId, result] of results) {
          ...
          if (databaseService && databaseService.logClinicalProcessing) {
            try {
              await databaseService.logClinicalProcessing({
                document_id: docId,
                document_type: docType,
        ...
        if (monitoringService) {
          ...
          monitoringService.recordClinicalProcessing?.(
            ...
            totalQualityScore / resultsArray.length
          );
        }
        ...
        return {
          ...
          batch_summary: {
            ...
            processing_success_rate:
              documents.length > 0 ? successfulProcessing / documents.length : 0,
  ...
  fastify.get(
    ...
    async () => {
      ...
      try {
        const status = clinicalNLPProcessor.getProcessingStatus
          ? await clinicalNLPProcessor.getProcessingStatus()
          : { status: 'unknown', activeProcessors: [], queueLength: 0, capacity: {} };
        const metrics =
          monitoringService && monitoringService.getCurrentMetrics
            ? await monitoringService.getCurrentMetrics()
            : {};
  ...
  fastify.get(
    ...
    async (
      request: FastifyRequest<{ Querystring: { timeframe?: string; document_type?: string } }>
    ) => {
      ...
      try {
        const stats =
          databaseService && databaseService.getClinicalProcessingStats
            ? await databaseService.getClinicalProcessingStats(timeframe, document_type)
            : {};
        return {
          timeframe,
