...
export async function structuredExtractionRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/structured-extraction', {
    schema: {
    ...
    try {
      ...
      const structuredDataExtractor = (fastify as any).structuredDataExtractor;
      ...
      return {
        ...
        document_sections: options.include_sections !== false ? result.documentSections : undefined,
        extraction_summary: {
  ...
  fastify.post('/structured-extraction/batch', {
    schema: {
      tags: ['Structured Data Extraction'],
  ...
  }, async (request: FastifyRequest<{ Body: BatchStructuredExtractionRequest }>, reply: FastifyReply) => {
    try {
      ...
      const results = await structuredDataExtractor.batchExtractStructuredData(documents, options);
      ...
      for (const [docId, result] of results) {
        ...
        if (databaseService) {
          try {
            ...
            fastify.log.warn(`Failed to log structured extraction for ${docId}: ` + String(error));
          }
        }
      }
      ...
      return {
        ...
        aggregate_summary: {
          ...
          average_data_completeness: results.size > 0 ? totalDataCompleteness / results.size : 0,
  ...
  fastify.get('/structured-extraction/templates', {
    schema: {
      tags: ['Structured Data Extraction'],
      ...
      templates: [
        {
          ...
          fields: ['patient_demographics', 'chief_complaint', 'history_of_present_illness', 'medications', 'allergies', 'vital_signs', 'physical_examination', 'assessment_and_plan'],
        ...
        },
        {
          ...
          fields: ['patient_demographics', 'admission_diagnosis', 'discharge_diagnosis', 'procedures', 'medications', 'follow_up_instructions'],
        ...
        },
        {
          ...
          fields: ['patient_demographics', 'procedure', 'surgeon', 'anesthesia', 'findings', 'complications', 'postoperative_plan'],
        ...
        },
        {
          ...
          fields: ['patient_demographics', 'test_results', 'reference_ranges', 'abnormal_values', 'interpretation'],
        ...
        },
        {
          ...
          fields: ['patient_demographics', 'examination_type', 'technique', 'findings', 'impression', 'recommendations'],
  ...
  fastify.get('/structured-extraction/stats', {
    schema: {
      tags: ['Structured Data Extraction'],
