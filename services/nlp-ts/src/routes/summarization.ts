...
export async function summarizationRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/summarization', {
    ...
    try {
      ...
      const clinicalSummarizer = (fastify as any).clinicalSummarizer;
      ...
      return {
        ...
        processing_time: processingTime,
        summaries: {
          extractive: options.summary_type !== 'abstractive' ? result.extractiveSummary : undefined,
          abstractive: options.summary_type !== 'extractive' ? result.abstractiveSummary : undefined
        },
        clinical_insights: options.include_key_points !== false ? result.clinicalInsights : undefined,
        metadata: {
          original_length: text.length,
          summary_length: result.summaryLength,
          compression_ratio: result.compressionRatio,
  ...
  // Batch summarization
  fastify.post('/summarization/batch', {
    schema: {
      tags: ['Clinical Summarization'],
        ...
        properties: {
          ...
          options: {
            ...
            properties: {
              ...
              summary_type: { type: 'string', enum: ['extractive', 'abstractive', 'both'], default: 'both' },
    ...
    try {
      ...
      if (monitoringService) {
        ...
        monitoringService.recordSummarization(totalCompressionRatio / results.size);
      }
      ...
      return {
        ...
        summary: {
          average_compression_ratio: results.size > 0 ? totalCompressionRatio / results.size : 0,
  ...
  fastify.get('/summarization/history/:documentId', {
    schema: {
      tags: ['Clinical Summarization'],
    ...
    try {
      ...
      const history = clinicalSummarizer.getSummarizationHistory(documentId);
  ...
  fastify.get('/summarization/stats', {
    schema: {
      tags: ['Clinical Summarization'],
    ...
    try {
      const stats = await databaseService.getSummarizationStats(timeframe);
      return {
        timeframe,
        ...stats
      };
  ...
  fastify.post('/summarization/quick', {
    schema: {
      tags: ['Clinical Summarization'],
  ...
  }, async (request: FastifyRequest<{ Body: { text: string; max_sentences?: number } }>, reply: FastifyReply) => {
    try {
