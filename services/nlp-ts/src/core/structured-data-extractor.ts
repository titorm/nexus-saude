...
import type { StructuredClinicalData, VitalSigns, MedicationInfo, AllergyInfo, ReviewOfSystems, PhysicalExamFindings, TreatmentPlan } from './clinical-nlp-processor.js';
...
export class StructuredDataExtractor {
  ...
  private async loadSectionHeaders(): Promise<void> {
    const sections = new Map([
      ...
      ['physical_exam', [
        /(?:physical\s+exam(?:ination)?|pe)[:\s]*$/i,
        /(?:examination)[:\s]*$/i
      ]],
      ['medications', [
        /(?:medications?|meds?|drugs?)[:\s]*$/i,
        /(?:current\s+medications?)[:\s]*$/i
      ]],
      ['allergies', [
        /(?:allergies|drug\s+allergies)[:\s]*$/i,
        /(?:adverse\s+reactions?)[:\s]*$/i
      ]],
      ['review_of_systems', [
        /(?:review\s+of\s+systems?|ros)[:\s]*$/i,
        /(?:systems?\s+review)[:\s]*$/i
      ]]
    ]);
  ...
  async extractStructuredData(
    ...
    try {
      logger.debug(`Extracting structured data from document ${documentId || 'unnamed'} (${text.length} characters)`);
      ...
      const extractionConfidence = this.calculateExtractionConfidence(structuredData, fieldsExtracted);
      ...
      logger.debug(`Structured extraction completed in ${processingTime}ms (${fieldsExtracted.length} fields extracted)`);
      return result;
  ...
  private async extractFields(text: string, sections: Map<string, string>): Promise<StructuredClinicalData> {
      ...
      try {
        ...
        for (const [sectionName, sectionContent] of sections) {
          for (const regex of pattern.patterns) {
            ...
            if (match) {
              (structuredData as any)[fieldName] = pattern.processor(match, sectionContent);
              extracted = true;
              break;
            }
          }
        ...
        if (!extracted) {
          for (const regex of pattern.patterns) {
            ...
            if (match) {
              (structuredData as any)[fieldName] = pattern.processor(match, text);
              break;
            }
          }
        }
  ...
  private async extractVitalSigns(text: string, sections: Map<string, string>): Promise<VitalSigns> {
  ...
  private async extractMedications(text: string, sections: Map<string, string>): Promise<MedicationInfo[]> {
    ...
    while ((simpleMatch = simpleMedPattern.exec(medicationSection)) !== null) {
      const existing = medications.find(med => 
        med.name.toLowerCase() === simpleMatch![1].toLowerCase()
      );
  ...
  private async extractAllergies(text: string, sections: Map<string, string>): Promise<AllergyInfo[]> {
    ...
    allergyFormats.forEach(pattern => {
      ...
      while ((match = pattern.exec(allergySection)) !== null) {
        ...
        if (match[1]) {
          const existing = allergies.find(allergy => 
            allergy.allergen.toLowerCase() === match![1].toLowerCase()
          );
  ...
  private async extractReviewOfSystems(text: string, sections: Map<string, string>): Promise<ReviewOfSystems> {
    ...
    systemPatterns.forEach(([system, pattern]) => {
      ...
      if (match) {
        (ros as any)[system] = match[1].trim();
      }
    });
  ...
  private async extractTreatmentPlan(text: string, sections: Map<string, string>): Promise<TreatmentPlan[]> {
